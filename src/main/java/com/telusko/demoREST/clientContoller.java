...
public class clientContoller {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Path("/{clientID}")
	public boolean updateClient(@PathParam("clientID") int id, Client client) throws SQLException, ClassNotFoundException {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Path("/{clientID}")
	public boolean deleteAuthor(@PathParam("clientID") int id) throws SQLException, ClassNotFoundException {
	...
	public javax.ws.rs.core.Response Login(LoginUser log) {
		...
		try {
			if(srv.Login(log)!="") {
				return javax.ws.rs.core.Response.ok(srv.Login(log)).entity(javax.json.Json.createValue(srv.Login(log))).build();
			}
			...
			return javax.ws.rs.core.Response.status(Status.CONFLICT).entity(javax.json.Json.createValue("Wrong username or password")).build();
		}
		return javax.ws.rs.core.Response.status(Status.CONFLICT).header("Access-Control-Allow-Origin", "*").entity("Wrong password!!!")
